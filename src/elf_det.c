...
static int elfdet_show(struct seq_file *m, void *v)
{
	...
	sscanf(buff, "%d",
	       &user_pid); // type cast pid from user(buff) to integer
	task =
	    pid_task(find_vpid(user_pid), PIDTYPE_PID); // get the task from pid
...
{
	static int finished =
	    0; // normal return value other than '0' will cause loop
	int ret = 0;
...
{
	long ret;
	ret = strncpy_from_user(buff, buffer,
				sizeof(buff) -
				    1); // copy the charachters to buff (global
					// buffer, inorder to use it in kernel)
	if (ret < 0)
		return ret;
...
{
	elfdet_dir = proc_mkdir(
	    "elf_det", NULL); // creating the directory: elf_det in proc

	if (!elfdet_dir) {
		return -ENOMEM;
	}
	// 0777 means full premmisions for the file
	elfdet_det_entry = proc_create(
	    "det", 0777, elfdet_dir,
	    &elfdet_det_ops); // create proc file det with elfdet_det_ops
	printk("det initiated; /proc/elf_det/det created\n");
	elfdet_pid_entry =
	    proc_create("pid", 0777, elfdet_dir,
			&write_pops); // create proc file pid with write_pops
